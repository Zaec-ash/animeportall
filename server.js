...
function isAuthenticated(req, res, next) {
  if (req.session && req.session.userId) {
    return next();  // Proceed to the next route if the user is authenticated
  }
...
app.get('/dashboard', isAuthenticated, (req, res) => {
  // Your code to render the dashboard or send the dashboard data
...
function isAuthenticated(req, res, next) {
  ...
  return res.status(401).json({ success: false, message: 'You must be logged in to access this page.' });
}
